...
return function (req, res, go)
    ...
    if failure == _G.SESSIONS.SESSION_OK then
        ...
        if usersess ~= nil and data ~= nil then
            
            -- Submit form html to be stored in the redis.
            if usersess.validated == true then
    
                local html = data.textareaValue
                _G.HTMLPAGES.addpage('about-page-default.html', html)
                
                if data.businessEmail then 
                    redis.setkey(oots_admin_email, data.businessEmail)
                    _G.OOTS_EMAIL = data.businessEmail
                end
            end
        end
        ...
        res.body = template.render( "/admin/admin-settings.html" )
        return
    end
    -- Nothing to really return
    res.code = 201
