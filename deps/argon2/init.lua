...
-- TODO: Use the OS detect in luajit


if _G.PLATFORM.os == 'windows' then 
    if _G.PLATFORM.arch == 'x86' then
        lib = ffi.load "deps/argon2/argon2optdll32.dll"
    else
        lib = ffi.load "deps/argon2/argon2optdll.dll"
    end
end

if _G.PLATFORM.os == 'linux'  then
lib = ffi.load "deps/argon2/libargon2.so"
end
...
function _M.makepwd(pwd)
    local fullkey = _M.hash_encoded(pwd, "kakutaiPtyLtd", nil)
    local key = string.match(fullkey, ".+,p=.*$.*$(.*)")
    return key
end  
